...
def read_lines(path: Path) -> List[str]:
    ...
    # preserve newlines as natural boundaries
    # strip trailing whitespace to keep things clean
...
class NGramLM:
    """
    Classic n-gram language model with multiple smoothing options
    Supports Laplace smoothing, backoff, and interpolation methods
    """
    ...
    def __init__(self, n: int, delta: float, vocab_size: int, 
        ...
        if interpolation_lambdas is None:
            # equal weight to each n-gram order
            self.lambdas = [1.0 / n] * n  
        else:
            assert len(interpolation_lambdas) == n
            assert abs(sum(interpolation_lambdas) - 1.0) < 1e-10 
...
def sweep_and_select_topK(merges_grid: List[int], n_values_grid: List[int], top_k: int = 3) -> None:
    """
    Try lots of different tokenizer sizes and n-gram orders to find the best combo
    """
    results = []
    ...
    for merges in merges_grid:
        ...
        # train the tokenizer only on training data
        ...
        # save this tokenizer so we can use it later
        tag = f"bpe_{merges:04d}"
        TOKENIZER_DIR.mkdir(parents=True, exist_ok=True)
        tok.save(str(TOKENIZER_DIR), tag)

        # test this tokenizer with different n-gram orders
...
def evaluate_generation(merges: int, n: int, smoothing_method: str, seed_texts: List[str]) -> None:
    ...
    if smoothing_method == "interpolation":
        # adjust for different n values
        lambdas = [0.1, 0.2, 0.3, 0.4][:n]  
        # normalize
